"""
Eval 3 — Pre-meeting brief quality.

A different task from profile extraction: each engine is hit at its RAW web-results
endpoint, and a Sonnet synthesizer turns the results into a pre-meeting sales brief
(notes + conversation questions). An Opus 4.8 judge then scores each brief the way a
salesperson would — which brief actually preps me better to walk into the meeting?

Rubric (weighted, ground-truth anchored):
    overall = freshness*0.35 + specificity*0.30 + actionability*0.35
    ... then capped at 15 if the brief is about the wrong person / empty.
  - freshness     recent, time-relevant intel (recent posts, current move, current events)
  - specificity   concrete GROUNDED detail (names/numbers/dates); fabrication subtracts
  - actionability sharp, tailored questions vs generic boilerplate

Reads:
  data/brief_outputs_100.csv  — each engine's synthesized brief per person
                                (provider,endpoint,query,about_target,n_notes,n_questions,notes,questions,response,params)
  data/ground_truth_500.csv   — verified current company/title per linkedin_url

Caching:
  If results/brief_judge_raw.json exists, the judge verdicts are reused (no API calls)
  and only the deterministic scoring + Excel are rebuilt. Delete that file (or pass
  --rejudge) to re-run the Opus judge (requires ANTHROPIC_API_KEY).

Writes:
  results/brief_judge_raw.json   — per-person judge verdicts (cache)
  results/meeting_brief_judge.xlsx
...
def overall(v):
    """Deterministic weighted score from the judged sub-dimensions (the model's own
    'overall' is NOT trusted — it mis-applies the gate). Gate: wrong person -> <=15."""
...
# ----------------------------------------------------------------------------- Opus judge (only when (re)judging)
...
def run_judge(people, gt):
    ...
    results = {}
    if os.path.exists(RAW):  # resume: keep good prior verdicts
