...
A targeted subset (n=57): every person here RECENTLY changed companies (most with a
current-role start of the current month). Each engine gets the person's *old* role and is
asked, via its native structured output, for their CURRENT employer/title and whether they
changed companies. The test: does the engine surface the NEW job (fresh) or the OLD one (stale)?

Schema each engine fills: found, full_name, current_company, current_title, current_start_date,
previous_company, previous_title, job_changed, change_summary.

Why an LLM judge: company names appear in different languages, transliterations, and
abbreviations across sources (e.g. an Arabic company name vs its English equivalent), which
defeats string matching. An Opus 4.8 judge classifies each engine against ground truth:
...
plus detected_change = did it correctly flag that a move happened.

Reads:
  data/jc_outputs_57.csv               — each engine's structured response per person
  data/job_change_ground_truth_57.csv  — verified new + previous role per person

Caching:
  Judge verdicts are cached in results/jc_judge_raw.json, so scores reproduce with NO API calls.
  Pass --rejudge (needs anthropic + ANTHROPIC_API_KEY) to re-run the judge.

Writes:
  results/jc_judge_raw.json, results/freshness_judge.xlsx
...
# ----------------------------------------------------------------------------- Opus judge (only with --rejudge / no cache)
