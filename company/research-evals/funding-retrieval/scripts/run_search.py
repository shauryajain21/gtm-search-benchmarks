#!/usr/bin/env python3
"""
Shared parallel search runner for the linkup / exa / perplexity / parallel agents.

Runs a batch of queries against ONE provider + ONE endpoint, concurrently, while
respecting that provider's QPS limit, and appends rows to a per-provider CSV:

    provider,endpoint,query,response,params

EVERY endpoint and output type is selectable, and EVERY request parameter is
tweakable via --params (merged into the request body). Use --list to see the
endpoints/output types, and --dry-run to confirm the exact request bodies BEFORE
sending anything (agents confirm the output type with the user this way first).

Each provider writes its own partial file (results/<provider>.csv) so all four
agents can run fully in parallel with no write contention. merge_results.py then
combines the partials into one sorted combined CSV.

Dependency-free: stdlib only (urllib + threads + a token-bucket rate limiter).
API keys load from the environment OR from ~/.claude/agents/scripts/.env.

------------------------------------------------------------------------------
USAGE
------------------------------------------------------------------------------
  python run_search.py --list
  python run_search.py --provider linkup --endpoint search --queries q.txt --dry-run
  python run_search.py --provider linkup --endpoint search --queries q.txt \
      --params '{"depth":"deep","outputType":"searchResults"}'
  python run_search.py --provider exa --endpoint answer --queries-json batch.json
  python run_search.py --provider parallel --endpoint task \
      --params '{"processor":"core"}' --queries q.txt --max-wait 900
"""
...
# --------------------------------------------------------------------------
# Body builders + response extractors (per endpoint). `query` is the query
# string (or a URL for fetch/contents). `params` is merged in and overrides.
# --------------------------------------------------------------------------
...
# --- structured-output schema injection + extraction (per the engine mapping) ---
...
PROVIDERS = {
    "linkup": {
        ...
        "qps": 25,  # this org's elevated limit (public default is 10)
...
class RateLimiter:
    """Blocks so request *starts* never exceed qps/sec."""
        ...
        if ep["kind"] == "task_poll":
            ...
            while time.monotonic() < deadline:
                try:
                    ...
                    if e.code == 408:  # still running, keep long-polling
                        continue
                    raise
            if resp is None:
...
def main():
    ...
    # --- confirm step: show the exact bodies, do not send ---
    if args.dry_run:
