#!/usr/bin/env python3
"""
Plot the funding-retrieval scorecard as a grouped bar chart.

Reads results/scorecard.csv and writes results/scorecard.png:
- grouped bars: share of companies within 10% / 25% / 50% of the golden value, per API

The winning engine (top within-25%) is spotlighted in a saturated color while the
others are muted to gray, so the leader reads instantly. Nothing is written onto the
plot area (no value labels or callouts) — the numbers live in the README tables. The
y-axis stays at a truthful 0-100, never a truncated scale.

Usage:
  python3 plot_results.py            # ../results/scorecard.csv -> ../results/scorecard.png
  python3 plot_results.py --in path.csv --out path.png
"""
import argparse
import csv
import os
...
# Winner gets a vivid blue ramp across the three bands; everyone else is muted gray.
...
def main():
    ...
    # Spotlight the winner with color only — emphatic tick label, no text on the bars.
    ax.set_xticks(x)
